...
if [ -z "$CACHED_RESULT" ]; then
  ...
  FRESH_RESULT=$(psql -h "$PG_HOST" -U "$PG_ADMIN_USER" -d "$DB_NAME" -v query="$USER_QUERY" -t -A -c "
  SELECT r.review_id || ' | ' || r.product_id || ' | ' || r.review_text || ' | ' || r.sentiment_label
  FROM product_reviews r
  JOIN review_embeddings e ON r.review_id = e.review_id
  ORDER BY e.embedding <=> (azure_openai.create_embeddings('$OPENAI_EMBED_DEPLOYMENT', :'query'))::vector
  LIMIT 5;
  ")
  echo "$FRESH_RESULT"
