...
step "STEP 15: Writing query_nosql_db.sql"
cat > query_nosql_db.sql << 'PYEOF'
"""
...
SELECT
    c.productName,
    AVG(c.reviewRating) AS avgRating,
...
SELECT
    ...
    c.reviewText
FROM c
WHERE c.reviewRating <= 3

--*************************************************************************
-- Lexical Search (Exact Keyword Matching)
-- Searches for documents containing a specific word or phrase.
-- Useful when you know the exact term you're looking for.
--*************************************************************************

-- Find reviews that explicitly contain the word "battery"
SELECT *
FROM c
WHERE CONTAINS(LOWER(c.reviewText), "battery")


--*************************************************************************
-- Lexical Search (Prefix Match)
-- Searches for values that start with a specific string.
-- Useful for autocomplete, type-ahead search, or filtering by ID prefixes.
--*************************************************************************

-- Find reviews whose text begins with the word "the"
SELECT
    c.productName,
    c.reviewText
FROM c
...
-- Find products whose product ID starts with "SKU-BIKE"
SELECT *
FROM c
WHERE STARTSWITH(c.productId, "SKU-BIKE")


--*************************************************************************
-- Lexical Search (Suffix Match)
-- Searches for values that end with a specific string.
-- Useful for matching name endings, file extensions, or code suffixes.
--*************************************************************************

-- Find customers whose last name is Lee
SELECT *
FROM c
WHERE ENDSWITH(c.customerName, "Lee")



--*************************************************************************
-- Semantic Search (Vector Similarity)
-- Finds customer reviews discussing battery life, durability,
-- quality, reliability, or product issues, even when exact
-- keywords are not present in the review.
--*************************************************************************
...
Natural Language Question
      |
      v
"Which customers complained about battery life or durability?"
      |
      v
...
(VectorDistance)
      |
      v
Most semantically relevant reviews
...
*/
PYEOF
echo "query_nosql_db.sql written."



###############################################################################
# STEP 15 — Save environment variables for webapp
###############################################################################
...
step "STEP 17: Setting up Python virtual environment"
...
# QUERY AZURE COSMOSDB CONTAINER
  # Explore the UI
  # cosmos-nosql-rag-webapp\cosmosdb\query_nosql_db.sql
...
###############################################################################
step "STEP 18: Running sample RAG queries"
...
# delete this folder: cosmos-nosql-rag-webapp\cosmosdb
