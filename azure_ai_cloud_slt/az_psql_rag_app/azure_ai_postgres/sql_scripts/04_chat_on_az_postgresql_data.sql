...
WITH query_embedding AS (
    SELECT azure_openai.create_embeddings('text-embedding-3-small', 'I’m looking for a lightweight laptop with long battery life.')::vector AS embedding
)
...
DECLARE
    query_embedding VECTOR := (
        SELECT azure_openai.create_embeddings('text-embedding-3-small', search_text)
    );
BEGIN
    RETURN QUERY
    SELECT P.name, P.category, P.description,
