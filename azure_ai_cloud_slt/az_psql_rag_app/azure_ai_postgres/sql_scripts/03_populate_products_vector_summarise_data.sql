...
SELECT
    P.productid,
    azure_openai.create_embeddings('text-embedding-3-small', P.name) AS embedding
FROM products AS P;
...
-- Aggregate all review text per product into a single combined string
WITH review_data AS (
    SELECT
        p.productid,
...
-- Generate abstractive summary + sentiment analysis for each product’s combined reviews
summary_data AS (
    SELECT
        r.productid,
        array_to_string(
            azure_cognitive.summarize_abstractive(
                r.combined_reviews,
                'en',
                throw_on_error => false, --This tells the Azure Cognitive function NOT to fail if the summarization model encounters an error.
                sentence_count => 2 --This instructs the abstractive summarization model to produce a summary containing exactly 2 sentences.
            ),
            ' '
...
-- Insert summary + sentiment scores into the final reporting table
