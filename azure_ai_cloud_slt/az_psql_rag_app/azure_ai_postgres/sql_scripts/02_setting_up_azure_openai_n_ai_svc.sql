...
DO $$
DECLARE
    endpoint TEXT := 'https://{your_azure_openai_name}.openai.azure.com/';
    api_key TEXT := '{your_azure_openai_api_key}';
BEGIN
...
SELECT azure_openai.create_embeddings('text-embedding-3-small','I love this demo') AS embeddings;
...
DO $$
DECLARE
    endpoint TEXT := 'https://{your_azure_language_name}cognitiveservices.azure.com/';
    api_key TEXT := '{your_azure_language_api_key}';
BEGIN
...
SELECT  azure_cognitive.analyze_sentiment('i do notLOVE THIS DEMO', 'en') AS sentiment
